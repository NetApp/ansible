...
3. Currently inventory files must be in YAML format. INI format is not supported until next release.
...
|-------------------------------------|----------|---------------|--------------------------------------------------------------------------------------------------------------------------------|
| sf_cluster_inventory_group          | No       | 'all'         | Use this to specify an inventory group of hosts that represents the cluster being upgraded.                                    |
|-------------------------------------|----------|---------------|--------------------------------------------------------------------------------------------------------------------------------|
| sf_pip_extra_args                   | No       | N/A           | Specify extra Python pip installer arguments when installing the required controller libraries/modules                         |
|-------------------------------------|----------|---------------|--------------------------------------------------------------------------------------------------------------------------------|
| sf_fail_on_inventory_mismatch       | No       | True          | Whether to fail the play if there are more nodes in the cluster than what the inventory includes (Default is to fail).         |
|                                     |          |               | When set to False, users can chose to specify in the inventory a subset of the cluster nodes to be upgraded.                   |
|                                     |          |               | WARNING: Don't put non-cluster nodes in the inventory. That may cause failure and the upgrade role being stopped prematurely.  |
|-------------------------------------|----------|---------------|--------------------------------------------------------------------------------------------------------------------------------|
